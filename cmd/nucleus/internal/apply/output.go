...
func renderHuman(stdout io.Writer, stderr io.Writer, evidence map[string]any) {
	...
	if mode := stringField(evidence, "mode"); mode != "" {
	...
	_, _ = fmt.Fprintf(stdout, "rollback_points: %d\n", valueLen(evidence["rollback_points"]))
	if diffs := valueLen(evidence["diffs"]); diffs > 0 {
		_, _ = fmt.Fprintf(stdout, "diffs: %d\n", diffs)
	}
}
